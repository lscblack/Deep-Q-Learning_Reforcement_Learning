...
Trains DQN on ALE/Breakout-v5 using Stable Baselines3
...
def create_dqn(hp, env, timesteps):
    ...
    try:
        ...
        print("Policy failed → fallback to CnnPolicy")
        return DQN("CnnPolicy", env, learning_rate=hp["lr"],
                   buffer_size=hp["buffer_size"], batch_size=hp["batch_size"], gamma=hp["gamma"], verbose=1)
...
def train_all_experiments():
    ...
    for idx, hp in enumerate(my_hp_table):
        ...
        try:
            ...
            results.append({"experiment": exp_id, "error": str(e)})
